...
import { sendChatMessage } from '../../services/ChatBotService.jsx';
import './ChatBot.css';
                            ...
                            >
                                ...
                                <div className="message-content">
                                    <div className="message-text">
                                        {message.text && !message.text.includes('*') && message.text}
                                    </div>

                                    {/* Display time slots as cards only */}
                                    {message.data && message.data.available_slots && message.data.available_slots.length > 0 && (
                                        <div className="slots-container">
                                            <div className="slots-header">
                                                ...
                                                <span className="slots-count">{message.data.total_slots} slots</span>
                                            </div>
                                                ...
                                                {message.data.available_slots.map((slot, idx) => (
                                                    ...
                                                    >
                                                        <div className="slot-time">
                                                            ...
                                                            <span className="time-label">{formatTime12Hour(slot.start)}</span>
                                                        </div>
                                                        <div className="slot-duration">
                                                            <span className="duration-text">{slot.end ? `to ${formatTime12Hour(slot.end)}` : '1 hour'}</span>
                                                        </div>
                                                        <div className="slot-status">
                                                            <span className="status-badge">
                                                                {slot.available_employees >= slot.required_employees ? 'Available' : 'Full'}
                                                            </span>
                                                        </div>
                                                    </button>
                                                ))}
