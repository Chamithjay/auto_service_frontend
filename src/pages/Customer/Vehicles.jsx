...
const Vehicles = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setVehicles(vehicles.filter(v => v.vehicleId !== deleteModal.vehicle.vehicleId));
    ...
    <CustomerDashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-[#14274E]">My Vehicles</h1>
            <p className="text-[#9BA4B4] mt-1">Manage your registered vehicles</p>
          </div>
          <button
        ...
        {vehicles.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
            <svg className="w-20 h-20 text-[#9BA4B4] mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" />
            </svg>
            <p className="text-[#9BA4B4] text-lg mb-4">No vehicles registered yet</p>
            <button
              onClick={() => navigate("/customer/vehicles/add")}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {vehicles.map((vehicle) => (
              <div key={vehicle.vehicleId} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-[#9BA4B4]/20">
                <div className="bg-gradient-to-br from-[#14274E] to-[#394867] p-6 text-white">
                  ...
                  <h3 className="text-xl font-bold mb-1">{vehicle.vehicleName}</h3>
                  <p className="text-[#9BA4B4] text-sm">{vehicle.registrationNo}</p>
                </div>
                ...
                <div className="p-6">
                  {vehicle.model && (
                    <div className="mb-4">
                      ...
                      <p className="font-semibold text-[#14274E]">{vehicle.model}</p>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                    <div>
                      ...
                      <p className="font-semibold text-[#14274E]">{vehicle.createdAt.split(' ')[0]}</p>
                    </div>
                    <div>
                      <p className="text-[#9BA4B4]">Updated</p>
                      <p className="font-semibold text-[#14274E]">{vehicle.updatedAt.split(' ')[0]}</p>
                    </div>
                  </div>

                  <div className="flex space-x-2">
                    <button
                      onClick={() => navigate(`/customer/vehicles/edit/${vehicle.vehicleId}`)}
        ...
        {deleteModal.isOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
              <h3 className="text-xl font-bold text-[#14274E] mb-4">Delete Vehicle</h3>
              <p className="text-[#394867] mb-6">
                Are you sure you want to delete{" "}
                <span className="font-bold">{deleteModal.vehicle?.vehicleName}</span> (
                {deleteModal.vehicle?.registrationNo})? This action cannot be undone.
              </p>
